...
[![Build Status](https://img.shields.io/travis/safecoin/bitcore-wallet-service.svg?branch=master&style=flat-square)](https://travis-ci.org/safecoin/bitcore-wallet-service)
[![Coverage Status](https://coveralls.io/repos/safecoin/bitcore-wallet-service/badge.svg?branch=master)](https://coveralls.io/r/safecoin/bitcore-wallet-service?branch=master)
...
See [Bitcore-wallet-client](https://github.com/safecoin/bitcore-wallet-client-safe) for the *official* client library that communicates to BWS and verifies its response. Also check [Bitcore-wallet](https://github.com/bitpay/bitcore-wallet) for a simple CLI wallet implementation that relies on BWS.
...
# Getting Started
```
 git clone https://github.com/safecoin/bitcore-wallet-service.git
...
BWS supports SSL and Clustering. For a detailed guide on installing BWS with extra features see [Installing BWS](https://github.com/safecoin/bitcore-wallet-service/blob/master/installation.md).
...
See [Bitcore Wallet Client](https://github.com/safecoin/bitcore-wallet-client-safe/blob/master/lib/api.js#L73) for implementation details.
...
Returns:
 * Wallet object. (see [fields on the source code](https://github.com/safecoin/bitcore-wallet-service/blob/master/lib/model/wallet.js)).
...
Returns:
 * List of pending TX Proposals. (see [fields on the source code](https://github.com/safecoin/bitcore-wallet-service/blob/master/lib/model/txproposal.js))

`/v1/addresses/`: Get Wallet's main addresses (does not include change addresses)

Returns:
 * List of Addresses object: (https://github.com/safecoin/bitcore-wallet-service/blob/master/lib/model/address.js)).  This call is mainly provided so the client check this addresses for incoming transactions (using a service like [Insight](https://insight.is)
...
Returns:
 * TX Proposal object. (see [fields on the source code](https://github.com/safecoin/bitcore-wallet-service/blob/master/lib/model/txproposal.js)). `.id` is probably needed in this case.


`/v1/addresses/`: Request a new main address from wallet

Returns:
 * Address object: (https://github.com/safecoin/bitcore-wallet-service/blob/master/lib/model/address.js)). Note that `path` is returned so client can derive the address independently and check server's response.
...
Returns:
 * TX Proposal object. (see [fields on the source code](https://github.com/safecoin/bitcore-wallet-service/blob/master/lib/model/txproposal.js)). `.status` is probably needed in this case.
